...
class slider_img extends CI_Controller {

	function __construct()
 	{
   		parent::__construct(); 
		$this->load->model('slider_img_model','',TRUE);
	...
	public function index()
	{
		...
		$data['result']		=	$this->slider_img_model->getAll();
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('slider_img_view',$data);
	...
	function addnew(){
		...
		if($this->uri->segment(3)=='Edit'){
			$data['result']		=	$this->slider_img_model->get_record($this->uri->segment(4));
		
		}
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('slider_img_add',$data);
		$this->load->view('comman/footer');	
	}
	function insertrecord(){
		...
		{	
			...
			if($files['img']['name']){
				...
				if($image_width < 770 || $image_height < 400){
					header('Location: '.base_url().'index.php/slider_img/addnew/Add/er');
					exit;
				}
			...
			$activities_code		=	$this->slider_img_model->addItem($data,'gen_cms');
		
			
			
		}
		header('Location: '.base_url().'index.php/slider_img');
		exit;
	}
	
	function deleterecord(){
		$result			=	$this->slider_img_model->get_image_name($this->uri->segment(3));
		...
		$this->slider_img_model->deletercd($this->uri->segment(3));
		header('Location: '.base_url().'index.php/slider_img');
		exit;
	}
