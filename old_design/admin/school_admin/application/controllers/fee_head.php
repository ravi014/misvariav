...
class fee_head extends CI_Controller {
	...
	function addnew($mode,$eid)
	{
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('fee_head_view',$data);
		$this->load->view('comman/footer');
	}
...
function insertrecord(){
		...
		if ($this->form_validation->run() == FALSE)
	 	{
			...
			$data['head_name']			=	data_filter($this->input->post('name'));
			...
			if($this->input->post('mode')=="Add")
			{
				...
				$user_type_id		=	$this->ObjM->addItem($data,'erp_fee_head');	
			}
			if($this->input->post('mode')=="Edit")
			{
				$data['update_by']		=	$this->session->userdata['logged_in_school']['usercode'];
				$data['update_date']	=	$nowdt;	
				 $this->ObjM->update($data,'erp_fee_head','id',$this->input->post('eid'));		
	...
	function record_update(){
		...
		$this->ObjM->update($data,'erp_fee_head','id',$this->uri->segment(4));	
