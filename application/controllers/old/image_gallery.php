...
class image_gallery extends CI_Controller {
	...
	function get_event_page(){
		...
		for($i=0;$i<count($result);$i++){	
			echo '<li>
					<a href="'.base_url().'index.php/gallery_list/view/'.$result[$i]["event_code"].'">
						<div id="imgdiv"><img src="'.base_url().'admin/upload/event_thum/'.$result[$i]["cover_img"].'" /><div id="event_title"><h2>'.$result[$i]["event_name"].'</h2></div></div>
					</a>
				</li>';
		 } 
		echo '</ul>';
